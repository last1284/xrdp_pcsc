...
int my_pcsc_trans_conn_in(struct trans *trans, struct trans *new_trans);

int my_pcsc_trans_data_in(struct trans *trans);

int scard_process_msg(struct trans *con, struct stream *in_s, int command);
...
int scard_process_cancel(struct trans *con, struct stream *in_s);

int scard_function_get_status_change_return(void *user_data,
                                        struct stream *in_s,
                                        int len, int status);

int scard_process_get_status_change(struct trans *con, struct stream *in_s);

int scard_function_status_return(void *user_data,
                             struct stream *in_s,
                             int len, int status);

int scard_process_status(struct trans *con, struct stream *in_s);

int scard_function_control_return(void *user_data,
                              struct stream *in_s,
                              int len, int status);

int scard_process_control(struct trans *con, struct stream *in_s);

int scard_function_transmit_return(void *user_data,
                               struct stream *in_s,
                               int len, int status);

int scard_process_transmit(struct trans *con, struct stream *in_s);
...
int scard_process_end_transaction(struct trans *con, struct stream *in_s);

int scard_function_begin_transaction_return(void *user_data,
                                        struct stream *in_s,
                                        int len, int status);

int scard_process_begin_transaction(struct trans *con, struct stream *in_s);

int scard_function_disconnect_return(void *user_data,
                                 struct stream *in_s,
                                 int len, int status);

int scard_process_disconnect(struct trans *con, struct stream *in_s);

int scard_function_connect_return(void *user_data,
                              struct stream *in_s,
                              int len, int status);

int scard_process_connect(struct trans *con, struct stream *in_s);

int scard_function_list_readers_return(void *user_data,
                                   struct stream *in_s,
                                   int len, int status);

int scard_process_list_readers(struct trans *con, struct stream *in_s);

int scard_function_release_context_return(void *user_data,
                                      struct stream *in_s,
                                      int len, int status);

int scard_process_release_context(struct trans *con, struct stream *in_s);

int scard_function_establish_context_return(void *user_data,
                                        struct stream *in_s,
                                        int len, int status);

int scard_process_establish_context(struct trans *con, struct stream *in_s);
        ...
        if (context != 0)
        {
            ...
            if ((llen != 0) && (return_code == 0))
            {
                ...
                for (index = 0; index < uds_client->numReaders; index++)
                {
                    ...
                    cardAtrLength = MIN(MAX_ATR_SIZE, uds_client->readerStates[index].cardAtrLength);
    ...
    if (status == 0)
    {
        ...
        if (cReaders > 0)
        {
            for (index = 0; index < cReaders; index++)
            {
                ...
                in_uint8a(in_s, uds_client->readerStates[index].cardAtr, 33);
        ...
        if (return_code == (int) SCARD_E_TIMEOUT) /* SCARD_E_TIMEOUT */
        {
            ...
            for (index = 0; index < uds_client->numReaders; index++)
            {
                ...
                rsa[index].current_state = uds_client->event_states[index] & ~2;
                rsa[index].event_state = uds_client->event_states[index];
                cardAtrLength = MIN(36, uds_client->readerStates[index].cardAtrLength);
                cardAtrLength = MAX(0, cardAtrLength);
                g_memcpy(rsa[index].atr, uds_client->readerStates[index].cardAtr, cardAtrLength);
                rsa[index].atr_len = cardAtrLength;
            }
